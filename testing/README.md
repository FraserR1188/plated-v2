...
|---|---|---|---|---|---|---|
| 2026-09-26 | self (dev pass; found in the receipt-scanner investigation, and PL-042's review of the staple seed) | investigation against master @ `177ed0a`; row counts measured on the live DB | none | 7 | 7 | `testing/2026-09-26-internal.md` |
...
| PL-042 | Major | `apple` staple is CoFID's cooking apple, aliased "eating apple" | Fixed | 2026-09-25-internal | Root cause: the review file's DISH? flag hid the plain row (apple, pear). Six remaps (apple 14-319, pear 14-321, lime 14-131, avocado 14-386, cabbage 13-582, coconut-milk 14-889), `bramley apple` dropped, no unitGrams. --only e56634a, seed 2d46c10. Drift check 08:30: A = 9 predicted apple columns, B/C/D empty. Importer run 2026-09-26 09:01 BST: 6 upserted; Q2 = predicted to the digit, Q3 = exactly six rows touched, post-diff 0 rows; the original diff re-run gave 39 rows (reproduced). Q1 171 (nothing inserted); verify.ts PASS, cofid 171. Needs a tester |
| PL-050 | Major | fetchEntries reads meal_entries in one unpaged request; PostgREST silently caps it at max_rows | In progress | 2026-09-26-internal | MEASURED 2026-09-26: user A 787 rows at 8.4/day from 2026-06-24; crosses 1000 around 2026-10-21 if hosted Max rows = 1000 (setting not yet confirmed). Hosted Max rows raised 1000 → 2000 the same day as a stopgap (dashboard, no deploy); A now crosses around mid-Feb 2027. Fix: fetchAllPages with .range(), ENTRIES_PAGE_SIZE 500, order logged_at desc + id desc; a failed page keeps the previous list and reports once; local max_rows 2000. Master @ be8ed92 (red) + eec75b9; 1033/1033, tsc 0; runtimes unchanged (c1907ba4… / 5359dcce…). Device pass OK (Pixel dev build at 61be9e2, 2026-09-26); CSV 279 rows / 30 local days, NULLs empty. First-load airplane cold start not run (not possible on dev build): owed on production after the OTA, and decides the first-load candidate. OTA owed. Sweep: fetchWorkouts is unbounded with no ORDER BY, filed as PL-056 |
| PL-056 | Major | fetchWorkouts reads biometric_workouts in one unpaged request with no ORDER BY; past max_rows it drops an arbitrary set | In progress | 2026-09-26-internal | Found in PL-050's sweep. Fix: fetchAllPages, order workout_start desc + origin_package + source_workout_id (the view's composite key); failed page keeps the previous list and reports once. Master @ fe61583 (red) + fix; 1045/1045, tsc 0; runtimes unchanged. Device pass OK with PL-050's (Pixel dev build at 61be9e2, 2026-09-26). OTA owed |
| PL-052 | Major | `passata` staple is CoFID's homemade tomato sauce (70 kcal, 4.5 g fat); CoFID has no passata row | Logged | 2026-09-26-internal | Any code is wrong, so remove it, but the importer only upserts: needs a delete path. Chicken rows confirmed not live (PL-042 drift check, section C empty), so passata is the only slug needing it |
| PL-055 | Major | Produce count weights in unit_grams are unsourced estimates; a medium lime (67 g) outweighs a medium lemon (58 g) | Logged | 2026-09-26-internal | onion, tomato, carrot, potato, bell-pepper, lemon, lime, banana. Proposed: remove (→ forced weight), reinstate per row with a UK source |
...
| PL-051 | Minor | No cooking-apple staple once `bramley apple` leaves `apple` | Logged | 2026-09-26-internal | Candidate: new staple on 14-362. Widens the set, so it's its own decision |
| PL-053 | Minor | `pepper` aliases both bell-pepper and black-pepper; staple lookup has no ORDER BY | Logged | 2026-09-26-internal | PREDICTED. black-pepper has NULL kcal, so a black-pepper hit falls through to OFF: the symptom is a pick that swaps between staple and OFF, not a wrong pepper. Also `green pepper` sits on the red-pepper row (13-524) |
| PL-054 | Minor | `lemon` staple is the whole fruit with peel (14-128) | Logged | 2026-09-26-internal | Ambiguous: zest, juice or flesh. Options: keep, 14-129, or split by use |

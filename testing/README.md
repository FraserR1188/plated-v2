...
|---|---|---|---|---|---|---|
| 2026-09-25 | self (read-only investigation) | no device run; source read against master @ `63be47c`, plus live OFF probes | none | 9 | 9 | `testing/2026-09-25-internal.md` |
...
| PL-039 | Major | Time chip rolls a forward plan on today back to yesterday, saved as eaten | Logged | 2026-09-25-internal | Fix on master, not device-passed: roll back only before 04:00 (SMALL_HOURS_END_H), and there to the nearer of today's/yesterday's reading (tie → today); pure shouldReadAsYesterday(todayReading, now). 998/998, tsc 0, 3 sabotage runs, fingerprints unchanged (0/158, 0/142). CLAUDE.md planned + roll-back wording corrected. Detection query with the user; no repair until the scale is known. Device checklist in the session doc |

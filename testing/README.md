...
| PL-056 | Major | fetchWorkouts reads biometric_workouts in one unpaged request with no ORDER BY; past max_rows it drops an arbitrary set | In progress | 2026-09-26-internal | Found in PL-050's sweep. Fix: fetchAllPages, order workout_start desc + origin_package + source_workout_id (the view's composite key); failed page keeps the previous list and reports once |

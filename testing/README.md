...
|---|---|---|---|---|---|---|
| 2026-09-25 | self (read-only investigation; PL-039 fix + device pass) | investigation against master @ `63be47c`, plus live OFF probes; PL-039 fixed at `ba9b8f8`, device-passed on a Pixel dev build | Pixel (PL-039 pass only) | 10 | 10 | `testing/2026-09-25-internal.md` |
...
| PL-037 | Major | Can't change an entry's meal when editing | Logged | 2026-09-21-kayce | Chips hidden on edit because of a stale comment (meal_type IS in MealEntryPatch). Agreed: chips on edit + pure changed-fields buildEditPatch; slot-only edit sends {meal_type} only. Closes most of PL-010 |

...
| PL-052 | Major | `passata` staple is CoFID's homemade tomato sauce (70 kcal, 4.5 g fat); CoFID has no passata row | Logged | 2026-09-26-internal | Any code is wrong, so remove it, but the importer only upserts: needs a delete path. Pair with the chicken cleanup if PL-042's section C finds chicken-breast/thigh live |

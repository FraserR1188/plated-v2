...
| PL-039 | Major | Time chip rolls a forward plan on today back to yesterday, saved as eaten | Logged | 2026-09-25-internal | resolveEatenAt's 3 h roll-back fires all day on ProductScreen (dayIsExplicit false for today). Decided: roll back only before 04:00, pure function, 12:00→19:00 sabotage test, CLAUDE.md planned/roll-back wording in the same commit. Detection query written, not yet run; no repair until the scale is known. First in the order of work |
